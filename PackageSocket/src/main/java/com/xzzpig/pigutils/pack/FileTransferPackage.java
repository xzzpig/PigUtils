...
import com.xzzpig.pigutils.file.ExtendFile;
import com.xzzpig.pigutils.json.JSONException;
...
public class FileTransferPackage extends WrapperPackage {
    ...
    private static void onFileTransferStartPackage(PackageSocket socket, Package pack) {
        ...
        File file = TransformManager.getDefaultManager().transform(File.class, json, null, null);
        byte b = 0;
        if (file != null) {
            boolean cont = true;
            if (!file.exists()) {
